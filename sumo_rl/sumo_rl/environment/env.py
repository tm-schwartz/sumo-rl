...
class SumoEnvironment(gym.Env):
    ...
    def _start_simulation(self):
        if self.sumo is not None:
            raise RuntimeError(
                "Simulation is already running or was not properly terminated."
            )
        timestamp = time.strftime("%Y%m%d_%H%M%S")  # Example: 20231207_143050
        tripinfo_file = f"eval_tripinfo_{timestamp}.xml"
        ssm_file = f"eval_ssm_{timestamp}.xml"
        sumo_cmd = [
            self._sumo_binary,
            "-n",
            self._net,
